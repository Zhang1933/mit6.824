...
func (rf *Raft) persist() {
	...
	e.Encode(rf.votedFor) // NOTE:记录这轮为谁投过票，防止重复投票
...
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	// Your code here (2D).
	if !rf.mu.TryLock() { // note:只能trylock，不然当正好在往管道传快照时会死锁。在安装快照的同时，申请Snapshot的场景应该不常见
		return
	}
	DPrintf(dSnap, "server%v收到Snapshot请求,idx%v,lastApplied%v,lastidx%v,loglen%v\n", rf.me, index, rf.lastApplied, rf.lastIncludedIndex, len(rf.log))
	defer rf.mu.Unlock()
	if rf.lastApplied+1 < index || index < rf.lastIncludedIndex { // rf.lastApplied+1=index时，已经可以创建快照了。
...
func (rf *Raft) handleInstallSnapshot(serverTo int, argsP *InstallSnapshotArgs) {
	...
	DPrintf(dLeader, "leader %v发送快照给server %v返回,更新floower的nexid:%v", rf.me, serverTo, rf.nextIndex[serverTo])
}
	...
	for !rf.killed() {
		...
		for rf.commitIndex > tmpApplied {
			...
			if rf.virtual2realidx(tmpApplied) >= len(rf.log) {
				DPrintf(dError, "server %v CommitChecker数组越界: tmpApplied=%v,  rf.RealLogIdx(tmpApplied)=%v>=len(rf.log)=%v, lastIncludedIndex=%v", rf.me, tmpApplied, rf.virtual2realidx(tmpApplied), len(rf.log), rf.lastIncludedIndex)
			}
			msg := &ApplyMsg{
				CommandValid: true,
		...
		for i := 0; i < len(rf.peers); i++ {
			...
			if rf.nextIndex[i]-1 < rf.lastIncludedIndex {
				...
				if rf.realidx2virtualidx(len(rf.log)) > rf.nextIndex[i] {
					...
					DPrintf(dCommit, "leader %v 向 server %v 添加新的Entries,Entries len:%v,follower nextidx:%v\n", rf.me, i, len(args.Entries), rf.nextIndex[i])
				}
...
func (rf *Raft) AppendEntriesReceive(argsP *AppendEntriesArgs, replyP *AppendEntriesReply) {
	...
	DPrintf(dLog, "server %v 收到leader %v的心跳,serverTerm%v,len(log)%v,lastlogterm%v,lastidx%v,argprevidx:%v,lastapply%v\n", rf.me, argsP.LeaderId, rf.currentTerm, len(rf.log), rf.log[len(rf.log)-1].Term, rf.lastIncludedIndex, argsP.PrevLogIndex, rf.lastApplied)
	rf.timeStamp = time.Now()
	...
	if rf.virtual2realidx(argsP.PrevLogIndex) < 0 {
		// NOTE:没有收到返回的包有这种可能，网络乱序，有可能发了两次，最新的先到，然后才是次新的。
		DPrintf(dTrace, "server %v leader发来的新日志中,有一部分日志被快照过Entries len%v", rf.me, len(argsP.Entries))
		if argsP.Entries == nil || rf.lastIncludedIndex-argsP.PrevLogIndex >= len(argsP.Entries) {
			replyP.Success = true
			replyP.Term = rf.currentTerm
			return
		} else {
			// 实际需要添加的日志项
			argsP.Entries = argsP.Entries[rf.lastIncludedIndex-argsP.PrevLogIndex:]
...
func (rf *Raft) InstallSnapshotReceive(argP *InstallSnapshotArgs, replyP *InstallSnapshotReply) {
	rf.mu.Lock()
	DPrintf(dSnap, "server %v 收到%v的InstallSnapshot,argLastidx%v,lastidx%v,log len%v\n", rf.me, argP.Leaderid, argP.Lastincludeidx, rf.lastIncludedIndex, len(rf.log))
	...
	if argP.Lastincludeidx <= rf.lastApplied {
		// 说明是旧的快照，不安装
		rf.mu.Unlock()
		return
	}
